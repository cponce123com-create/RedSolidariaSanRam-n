...
const SIZE_LABELS: Record<string, string> = { small: "petDetail.sizeSmall", medium: "petDetail.sizeMedium", large: "petDetail.sizeLarge", giant: "petDetail.sizeGiant" };
const AGE_LABELS: Record<string, string> = { puppy: "petDetail.agePuppy", adult: "petDetail.ageAdult", senior: "petDetail.ageSenior" };
const HEALTH_LABELS: Record<string, { labelKey: string; color: string }> = {
  excellent: { labelKey: "petDetail.healthExcellent", color: "text-green-700" },
  good: { labelKey: "petDetail.healthGood", color: "text-green-700" },
  fair: { labelKey: "petDetail.healthFair", color: "text-yellow-700" },
  needs_care: { labelKey: "petDetail.healthNeedsCare", color: "text-orange-700" },
};
const HOUSING_TYPES = [
  { value: "casa-propia", labelKey: "petDetail.housingOwnHome" },
  { value: "casa-alquilada", labelKey: "petDetail.housingRentedHome" },
  { value: "departamento", labelKey: "petDetail.housingApartment" },
  { value: "cuarto", labelKey: "petDetail.housingRoom" },
];
...
export default function PetDetail() {
  ...
  const adoptSchema = z.object({
    requesterName: z.string().min(2, t("petDetail.requiredName")),
    requesterAge: z.string().min(1, t("petDetail.requiredAge")),
    requesterPhone: z.string().min(7, t("petDetail.requiredPhone")),
    requesterEmail: z.string().email(t("donation.invalidEmail")).optional().or(z.literal("")),
    requesterAddress: z.string().min(5, t("petDetail.requiredAddress")),
    ...
    housingType: z.enum(["casa-propia", "casa-alquilada", "departamento", "cuarto"], {
      required_error: t("petDetail.housingRequired"),
    }),
    hasYard: z.boolean().default(false),
    adoptionReason: z.string().min(20, t("petDetail.reasonMin")),
  ...
  if (!pet) return (
    <div className="text-center py-20">
      <p className="text-muted-foreground">{t("petDetail.notFound")}</p>
      <Link href="/adopciones"><Button className="mt-4 rounded-xl">{t("petDetail.viewAdoptions")}</Button></Link>
    </div>
  );
    ...
    <div className="max-w-5xl mx-auto px-4 py-10 sm:py-16">
      ...
      <Link href="/adopciones">
        <button className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-8 transition-colors">
          <ArrowLeft className="w-4 h-4" /> {t("petDetail.backToCatalog")}
        </button>
      </Link>
      ...
      {pet.status === "adopted" && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-4 mb-6 flex items-center gap-3">
          ...
          <p className="text-green-800 font-semibold">{t("petDetail.adoptedBanner", { name: pet.name })}</p>
        </div>
      )}
      {pet.status === "in-process" && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-6 flex items-center gap-3">
          <Heart className="w-5 h-5 text-blue-600" />
          <p className="text-blue-800 font-semibold">{t("petDetail.inProcessBanner", { name: pet.name })}</p>
        </div>
      )}
      {pet.urgent && pet.status === "available" && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 mb-6 flex items-center gap-3">
          <AlertTriangle className="w-5 h-5 text-red-600 animate-pulse" />
          <p className="text-red-800 font-semibold">{t("petDetail.urgentBanner", { name: pet.name })}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                {pet.species === "perro" ? t("petDetail.speciesDog") : t("petDetail.speciesCat")}{pet.breed ? ` · ${pet.breed}` : ""}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              {[
                { icon: SpeciesIcon, label: t("petDetail.attrSex"), value: pet.sex === "macho" ? t("petDetail.sexMale") : t("petDetail.sexFemale") },
                { icon: Calendar, label: t("petDetail.attrAge"), value: `${AGE_LABELS[pet.ageCategory] ? t(AGE_LABELS[pet.ageCategory]) : pet.ageCategory}${pet.ageApprox ? ` (${pet.ageApprox})` : ""}` },
                { icon: Ruler, label: t("petDetail.attrSize"), value: SIZE_LABELS[pet.size] ? t(SIZE_LABELS[pet.size]) : pet.size },
                { icon: Star, label: t("petDetail.attrHealth"), value: healthInfo.labelKey ? t(healthInfo.labelKey) : pet.healthStatus, valueClass: healthInfo.color },
          ...
          <div className="flex flex-wrap gap-2">
            {pet.vaccinated && (
              <span className="flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-700 text-xs font-semibold px-3 py-1.5 rounded-full">
                <Syringe className="w-3.5 h-3.5" /> {t("petDetail.vaccinated")}
              </span>
            )}
            {pet.sterilized && (
              <span className="flex items-center gap-1.5 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-semibold px-3 py-1.5 rounded-full">
                <Scissors className="w-3.5 h-3.5" /> {t("petDetail.sterilized")}
              </span>
            )}
            {pet.dewormed && (
              <span className="flex items-center gap-1.5 bg-purple-50 border border-purple-200 text-purple-700 text-xs font-semibold px-3 py-1.5 rounded-full">
                <Shield className="w-3.5 h-3.5" /> {t("petDetail.dewormed")}
              </span>
            )}
          </div>
          ...
          {/* Description */}
          <div>
            <h3 className="font-bold text-base mb-2">{t("petDetail.aboutTitle", { name: pet.name })}</h3>
          ...
          {pet.history && (
            <div className="bg-amber-50/60 border border-amber-100 rounded-2xl p-4">
              <h3 className="font-bold text-sm mb-2 text-amber-800">{t("petDetail.historyTitle")}</h3>
          ...
          {pet.adoptionRequirements && (
            <div>
              <h3 className="font-bold text-base mb-2">{t("petDetail.requirementsTitle")}</h3>
          ...
          {pet.status !== "adopted" && !submitted && (
            ...
            >
              <Heart className="w-5 h-5 mr-2" /> {t("petDetail.adoptButton", { name: pet.name })}
            </Button>
          )}
          {submitted && (
            <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center">
              <Check className="w-8 h-8 text-green-600 mx-auto mb-2" />
              <p className="font-bold text-green-800">{t("petDetail.successTitle")}</p>
              <p className="text-green-700 text-sm mt-1">{t("petDetail.successDescription")}</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-background rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="p-6 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-display font-bold text-2xl">{t("petDetail.modalTitle")}</h2>
              ...
              <p className="text-muted-foreground mb-6 text-sm">
                {t("petDetail.modalIntroPrefix")} <strong>{pet.name}</strong> {t("petDetail.modalIntroSuffix")}
              </p>

              {submitted ? (
                <div className="text-center py-8">
                  <Check className="w-16 h-16 text-green-500 mx-auto mb-4" />
                  <h3 className="font-bold text-xl mb-2">{t("petDetail.successTitle")}</h3>
                  <p className="text-muted-foreground">{t("petDetail.modalContactSoon")}</p>
                </div>
              ) : (
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(v => submitRequest.mutate(v))} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <FormField control={form.control} name="requesterName" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("petDetail.formFullName")}</FormLabel>
                          <FormControl><Input placeholder={t("petDetail.formNamePlaceholder")} className="rounded-xl bg-secondary/30" {...field} /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={form.control} name="requesterAge" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("petDetail.formAge")}</FormLabel>
                          <FormControl><Input placeholder={t("petDetail.formAgePlaceholder")} className="rounded-xl bg-secondary/30" {...field} /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      <FormField control={form.control} name="requesterPhone" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("petDetail.formPhone")}</FormLabel>
                      ...
                      <FormField control={form.control} name="requesterEmail" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("petDetail.formEmail")}</FormLabel>
                    ...
                    <FormField control={form.control} name="requesterAddress" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("petDetail.formAddress")}</FormLabel>
                        <FormControl><Input placeholder={t("petDetail.formAddressPlaceholder")} className="rounded-xl bg-secondary/30" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    ...
                    <FormField control={form.control} name="housingType" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("petDetail.formHousing")}</FormLabel>
                        <div className="grid grid-cols-2 gap-2 mt-1">
                          {HOUSING_TYPES.map(ht => (
                            <button key={ht.value} type="button" onClick={() => field.onChange(ht.value)}
                              className={`px-3 py-2.5 rounded-xl text-sm text-left border-2 transition-colors font-medium ${field.value === ht.value ? "border-primary bg-primary/10 text-primary" : "border-border bg-secondary/30 text-muted-foreground"}`}>
                              <Home className="w-4 h-4 inline mr-1.5" />{t(ht.labelKey)}
                            </button>
                          ))}
                        </div>
                        <FormMessage />
                    ...
                    {form.watch("hasPetExperience") && (
                      <FormField control={form.control} name="previousPets" render={({ field }) => (
                        <FormItem>
                          <FormLabel>{t("petDetail.formPreviousPets")}</FormLabel>
                          <FormControl><Input placeholder={t("petDetail.formPreviousPetsPlaceholder")} className="rounded-xl bg-secondary/30" {...field} /></FormControl>
                        </FormItem>
                      )} />
                    )}
                    ...
                    <FormField control={form.control} name="adoptionReason" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("petDetail.formReason", { name: pet.name })}</FormLabel>
                        <FormControl>
                          <Textarea placeholder={t("petDetail.formReasonPlaceholder")} className="min-h-[100px] rounded-xl bg-secondary/30 resize-none" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    ...
                    <div className="flex items-center justify-between bg-primary/5 border border-primary/20 rounded-xl p-4">
                      <div>
                        <p className="font-semibold text-sm">{t("petDetail.formFollowUp")}</p>
                    ...
                    <Button type="submit" className="w-full h-12 rounded-xl" disabled={submitRequest.isPending}>
                      {submitRequest.isPending ? t("contact.sending") : <><Heart className="w-4 h-4 mr-2" /> {t("petDetail.submitButton")}</>}
                    </Button>
                  </form>
                </Form>
              )}
